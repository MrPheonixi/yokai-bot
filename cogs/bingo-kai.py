...
class Yokai(commands.Cog):
    ...
    async def bingo_yokai(self, ctx = commands.Context):
        ...
        #streak sytem 
        streak = brute_inventory.get("streak", [None, 0])
        if streak[0] == class_id:
            streak[1] += 1
        else:
            streak = [class_id, 1]
        brute_inventory["streak"] = streak
        ...
        if streak[1] > 1:
            best_streak = brute_inventory.get("best_streak", 0)
            yokai_embed.add_field(
                name="Série !",
                value=f"C'est votre **{streak[1]}**ème Yo-kai de rang **{class_name}** d'affilée !\nTa meilleure série est de **{best_streak}** yokai d'affilé.",
            )
        ...
        if verification == True:
            ...
            if class_id not in brute_inventory:
                brute_inventory[class_id] = 0
